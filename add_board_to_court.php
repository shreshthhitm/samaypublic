...
/*if(isset($_GET['del']) && is_numeric($_GET['del']) && $_GET['del'] != 0) {
	...
	$del_query = "UPDATE boards SET is_active=0 WHERE id=".$_GET['del'];
	if($myPDO->query($del_query)) {
		echo "<script>\n"; 
		echo "    window.location.href = '".ROOT_URL."/all_board_to_court.php?delete=1';\n"; 
		echo "</script>\n";
	} else {
		header("location: all_board_to_court.php");
	}
}*/
if(isset($_GET['edit']) && is_numeric($_GET['edit']) && $_GET['edit'] != 0) {
	$edit = sanitize($_GET['edit'], 'int');
	$myPDO->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$stmt_edit = $myPDO->query("SELECT * FROM boards WHERE id=".$edit);
...
if(isset($_POST['submit'])){
	...
	if($_POST['c_id'] != ''){
		...
		$q0 = "UPDATE boards SET court_type='".$court_type."', formation_id='".$formation_id."' WHERE id=".$c_id;
		$myPDO->query($q0);
		$inserted = 1;
	}else{
		$q0 = "INSERT INTO boards (court_type, formation_id) VALUES ('".$court_type."', '".$formation_id."')";
		$myPDO->query($q0);
		$inserted = 1;
	}
	...
	if ($inserted == 1) {
		echo "<script>\n"; 
		//echo "    window.location.href = 'all_board_to_court.php?".((isset($edit)) ? "update" : "insert" )."=1';\n"; 
		echo "    window.location.href = 'add_board_to_court.php?".((isset($edit)) ? "update" : "insert" )."=1';\n"; 
		echo "</script>\n";
	}
	else{
                                    ...
                                    <form action="" class="form-horizontal" name="promo_form" enctype="multipart/form-data" method="post" data-toggle="validator">
                                        <div class="form-body">
											...
											<div class="row">
												...
												<div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="control-label col-md-4">Board<span class="red">*</span></label>
                                                        <div class="col-md-8">
                                                            <select id="formation_id" name="formation_id" class="form-control" required>
																<?=formation_populate((isset($edit) ? $row_edit['formation_id'] : ''), 'board');?>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="col-md-8">
															...
															<select id="court_type" name="court_type" class="form-control" required>
																<?=court_populate(isset($edit) ? $row_edit['court_type'] : '');?>
															</select>
															<span class="help-block"></span>
                                                        </div>
                                                    </div>
